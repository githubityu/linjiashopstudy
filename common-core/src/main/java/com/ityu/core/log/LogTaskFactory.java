package com.ityu.core.log;



import com.ityu.bean.vo.SpringContextHolder;

import com.ityu.bean.constant.state.LogSucceed;
import com.ityu.bean.constant.state.LogType;
...
import com.ityu.utils.ToolUtil;
...
public class LogTaskFactory {
    ...
    public static TimerTask exitLog(final Long userId, final String ip) {
        return new TimerTask() {
            @Override
            public void run() {
                LoginLog loginLog = LogFactory.createLoginLog(LogType.EXIT, userId, "",ip);
                try {
                    loginLogRepository.save(loginLog);
                } catch (Exception e) {
