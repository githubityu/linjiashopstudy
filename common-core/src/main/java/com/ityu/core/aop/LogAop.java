...
import static com.ityu.utils.HttpUtil.getToken;
...
public class LogAop {
    ...
    @Pointcut(value = "@annotation(com.ityu.core.BussinessLog)")
    ...
    private void handle(ProceedingJoinPoint point) throws Exception {
        ...
        if (StringUtil.isNotEmpty(token)) {
            AdminUser adminUser = UserService.me().adminUserByToken(token);
            if (adminUser != null)
                idUser = adminUser.getId();
        }
        if (idUser == null) {
            return;
        }
